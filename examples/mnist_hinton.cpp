...
int main(int argc, char **argv) {
    ...
    for(auto i = 0; i < 3; i++){
        for(auto j = 0; j < 3; j++){
            ...
            std::cout << "Run time: "<< af::timeit(run_model) << std::endl;
            backend.close();
        }
    }
}
...
void build_model(md::AfBackend& backend){
    ...
    std::cout << "Compile time:" << ((double)(1000 * (clock() - start))) / ((double)(CLOCKS_PER_SEC)) << std::endl;
}
...
void run_model(){
    ...
    for(int i=0;i<n;i++){
        ...
        data_inv = {af::array(dat::MNIST_NUM_ROWS*dat::MNIST_NUM_COLS, args.batch_size,
