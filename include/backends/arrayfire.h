...
//    };
    ...
    public:
        ...
        void generate_source(std::string file_name, Graph graph,
            ...
            // Add the exception class for incorrect shapes
            print_shape_esception(f);
            // Function definition
...
//            f << "\taf_print(node_25);\n";
        ...
        void validate_input_shapes(std::ofstream& f, Graph graph, std::vector<Node> inputs){
            ...
            for(int i=0;i<inputs.size();i++){
                ...
                f << "\taf::dim4 node_" << node->id << "_expected_shape{";
                for(int j=0;j<4;j++){
                ...
                f << "};\n";
                f << "\tif(node_" << node->id << ".dims() != node_" << node->id << "_expected_shape){\n" <<
                "\t\t throw InvalidInputShape(" << node->id <<
                ", node_" << node->id << ".dims(), " <<
                "node_" << node->id << "_expected_shape);\n" <<
                "\t}\n";
            }
        }
