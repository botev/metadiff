...
public:
        ...
        for(auto nPtr : _graph->nodes) {//from member
            ...
            if (parentIds.empty()) {
                ...
                if (nodeMap.find(parentIds) == nodeMap.end()) {
                    ...
                    Node current = nPtr;
                    current.replace_parent_of_children(nodeMap[parentIds][nPtr->op->name]);
    ...
    void opt_neg_neg() {
        ...
        for(auto nPtr : _graph->nodes) {
            ...
            if (!node.is("Neg") or !node->active) continue;

            Node parent = node->op->get_parents()[0];

            if (!parent.is("Neg") or !node->active) continue;
